...
const CategoryChipFilter: React.FC<CategoryChipFilterProps> = ({
  ...
  const handleCategoryClick = (category: string) => {
    ...
    if (newCategory && scrollContainerRef.current) {
      const selectedElement = scrollContainerRef.current.querySelector(`[data-category="${category}"]`);
      if (selectedElement) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="relative">
        {/* 좌측 그라데이션 표시 */}
        <div className="absolute left-0 top-0 bottom-0 w-8 bg-gradient-to-r from-white to-transparent z-10 pointer-events-none opacity-0 transition-opacity duration-300" id="left-gradient" />
        
        {/* 우측 그라데이션 표시 */}
        <div className="absolute right-0 top-0 bottom-0 w-8 bg-gradient-to-l from-white to-transparent z-10 pointer-events-none opacity-100 transition-opacity duration-300" id="right-gradient" />
        ...
        >
          ...
          {categories.map((category) => (
            <div key={category} className="flex-shrink-0" data-category={category}>
              <CategoryChip
                category={category}
